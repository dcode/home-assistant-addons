...
if bashio::config.exists 'secrets_file'; then
  SECRETS_FILE="/addon_config/$(bashio::config 'secrets_file')"
  bashio::log.debug "Custom secrets file set to ${SECRETS_FILE}."
else
  # Default to secrets.yaml
  SECRETS_FILE="/addon_config/secrets.yaml"
fi
...
function generate_secret_files {
  for row in $(bw list items --organizationid "${BW_ORG_ID}" | jq -c '.[] | select(.type == 2) | [.name, (.notes|@base64)]'); do
    ...
    mkdir -p /addon_config/"${dirname}"
    rm -f /addon_config/"${dirname}"/"${basename}"

    echo "${row}" | jq -r '.[1] | @base64d' >"/addon_config/${dirname}/${basename}"
    chmod go-wrx "/addon_config/${dirname}/${basename}"
  done
}
